...
/**
 * This class wraps {@link SwerveGRROdometry Swerve Drive Odometry} to fuse latency-compensated
...
public class PoseEstimator {
  ...
  public PoseEstimator(
    ...
    m_odometry = new SwerveGRROdometry(kinematics, gyroAngle, modulePositions, initialPoseMeters);
  ...
  public void addVisionMeasurement(Pose2d visionRobotPoseMeters, double timestampSeconds) {
    ...
    m_poseBuffer.addSample(
        ...
        new InterpolationRecord(
            getEstimatedPosition(), sample.get().gyroAngle, sample.get().modulePositions, sample.get().gyroPitch, sample.get().gyroRoll));
    ...
    for (Map.Entry<Double, InterpolationRecord> entry :
      ...
      updateWithTime(entry.getKey(), entry.getValue().gyroAngle, entry.getValue().modulePositions, entry.getValue().gyroPitch, entry.getValue().gyroRoll);
    }
  }
  ...
  public Pose2d update(Rotation2d gyroAngle, SwerveModulePosition[] modulePositions, double pitch, double roll) {
    return updateWithTime(WPIUtilJNI.now() * 1.0e-6, gyroAngle, modulePositions, pitch, roll);
  }
  ...
  public Pose2d updateWithTime(
      double currentTimeSeconds, Rotation2d gyroAngle, SwerveModulePosition[] modulePositions, double pitch, double roll) {
    ...
    m_odometry.update(gyroAngle, internalModulePositions, pitch, roll);

    m_poseBuffer.addSample(
        currentTimeSeconds,
        new InterpolationRecord(getEstimatedPosition(), gyroAngle, internalModulePositions, pitch, roll));
  ...
  private class InterpolationRecord implements Interpolatable<InterpolationRecord> {
    ...
    private InterpolationRecord(
        Pose2d poseMeters, Rotation2d gyro, SwerveModulePosition[] modulePositions, double pitch, double roll) {
    ...
    public InterpolationRecord interpolate(InterpolationRecord endValue, double t) {
      if (t < 0) {
        ...
        double pitch = MathUtil.interpolate(this.gyroPitch, endValue.gyroPitch, t);
        
        double roll = MathUtil.interpolate(this.gyroRoll, endValue.gyroRoll, t);

        return new InterpolationRecord(poseMeters.exp(twist), gyro_lerp, modulePositions, pitch, roll);
      }
    }
